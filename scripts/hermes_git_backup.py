...
REMOTE_URL = os.environ.get("HERMES_BACKUP_REMOTE", "git@github.com:hhive/hermes.git")
BACKUP_BRANCH = os.environ.get("HERMES_BACKUP_BRANCH", "hermes-artifacts-backup")
GIT_TIMEOUT_SECONDS = int(os.environ.get("HERMES_BACKUP_GIT_TIMEOUT", "60"))
GIT_PROXY = os.environ.get("HERMES_BACKUP_PROXY", "http://127.0.0.1:7890")
...
def run(cmd: list[str], cwd: Path | None = None, check: bool = True, timeout: int | None = None) -> subprocess.CompletedProcess[str]:
    ...
    env = None
    if timeout is None and cmd and cmd[0] == "git":
        effective_timeout = GIT_TIMEOUT_SECONDS
        if GIT_PROXY:
            env = os.environ.copy()
            env.setdefault("HTTP_PROXY", GIT_PROXY)
            env.setdefault("HTTPS_PROXY", GIT_PROXY)
            env.setdefault("ALL_PROXY", GIT_PROXY)
            env.setdefault("http_proxy", GIT_PROXY)
            env.setdefault("https_proxy", GIT_PROXY)
            env.setdefault("all_proxy", GIT_PROXY)
...
def ensure_repo() -> None:
    ...
    if REPO_DIR.exists() and not (REPO_DIR / ".git").exists():
        remove_tree_with_retries(REPO_DIR)
    if not (REPO_DIR / ".git").exists():
        try:
            run(["git", "clone", "--depth", "1", REMOTE_URL, str(REPO_DIR)], check=True)
...
def commit_and_push() -> int:
    run(["git", "checkout", "-B", BACKUP_BRANCH], cwd=REPO_DIR)
    ...
    push = run(["git", "push", "-u", "origin", BACKUP_BRANCH], cwd=REPO_DIR, check=False)
